...
void handle_request(const httplib::Request& req, httplib::Response& res) {
  ...
  // 处理 ndnpoke 的执行结果
    ...
    // 转换为时间戳
   auto currentTimePoint1 = std::chrono::system_clock::now();
   auto timestamp2 = std::chrono::duration_cast<std::chrono::nanoseconds>(
        currentTimePoint1.time_since_epoch()
    ).count();
    auto timestamp3 = std::chrono::duration_cast<std::chrono::seconds>(
        currentTimePoint1.time_since_epoch()
    ).count();
    // 打印时间戳
     std::cout << "Current timestamp(s)1: " << timestamp2 << std::endl;

    std::cout << "Current timestamp1: " << timestamp3 << std::endl;
}
